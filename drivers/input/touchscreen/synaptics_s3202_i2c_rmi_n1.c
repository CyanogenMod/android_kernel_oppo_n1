...
/* OPPO 2013-08-16 ranfei Add begin for reason */
            ...
            if (ts->is_tp_suspended && 1 == atomic_read(&ts->music_enable))
			{
                if (double_tap & 0x02)
				{
					...
					input_report_key(ts->input_dev, KEY_PLAYPAUSE, 1);
					input_sync(ts->input_dev);
					input_report_key(ts->input_dev, KEY_PLAYPAUSE, 0);
					input_sync(ts->input_dev);
				}
            }
            ...
            if (ts->is_tp_suspended && 1 == atomic_read(&ts->music_enable))
			{
                if ((double_tap & 0x20) && (state[6] & 0x04))
				{
					...
					input_report_key(ts->input_dev, KEY_PREVIOUS, 1);
					input_sync(ts->input_dev);
					input_report_key(ts->input_dev, KEY_PREVIOUS, 0);
					input_sync(ts->input_dev);
				}
            }
            if (ts->is_tp_suspended && 1 == atomic_read(&ts->music_enable))
			{
				...
				{
					print_ts(TS_INFO, KERN_INFO "draw a > gesture\n");
					input_report_key(ts->input_dev, KEY_NEXT, 1);
					input_sync(ts->input_dev);
					input_report_key(ts->input_dev, KEY_NEXT, 0);
					input_sync(ts->input_dev);
				}
			}
...
//	       ts->snap_top,  ts->virtual_key_height);
    ...
    set_bit(KEY_PREVIOUS, ts->input_dev->keybit);
    set_bit(KEY_NEXT, ts->input_dev->keybit);
