...
int isc_fw_download(struct melfas_ts_data *info, const u8 *data, size_t len)
{
    u8 *buff;
    u16 crc_buf;
    int src_idx;
    ...
    *buff = ISC_ENTER_ISC_DATA;
    ret = i2c_smbus_write_byte_data(info->client, ISC_ENTER_ISC_CMD, *buff);
    if (ret < 0)
    {
    ...
    *buff = ISC_ENTER_UPDATE_DATA;
    ret = i2c_smbus_write_i2c_block_data(info->client, ISC_CMD, ISC_ENTER_UPDATE_DATA_LEN, buff);
    if (ret < 0)
    {
    ...
    *(buff + 1) = ISC_DATA_WRITE_SUB_CMD;
    for (i = 0; i < ISC_PKT_NUM; i++)
    {
        ...
        *(buff + ISC_PKT_DATA_SIZE + ISC_PKT_HEADER_SIZE) = crc_buf >> 8 & 0xFF;
        ...
        /* confirm CRC */
        ret = i2c_smbus_read_byte_data(info->client, ISC_CHECK_STATUS_CMD);
        if (ret == ISC_CONFIRM_CRC)
        {
            dev_info(&info->client->dev, "updating %dth firmware data packet.\n", i);
        }
        else
        {
